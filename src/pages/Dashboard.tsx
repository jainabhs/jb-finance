...
export default function Dashboard() {
  ...
  const portfolio = useMemo(() => {
    const map = new Map<
      string,
      { name: string; principal: number; loanCount: number; earned: number }
    >();
  ...
  const monthlyEarning = useMemo(
    () => activeLoans.reduce((s, l) => s + (l.principal * l.rate) / 100, 0),
    [activeLoans],
  );
    ...
    <div className="space-y-5 sm:space-y-6">
      ...
      >
        <div className="flex items-end justify-between">
          <div>
            <p className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white font-mono tracking-tight leading-none">
              {m(totalPrincipal)}
            </p>
            <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">
              given across {activeLoans.length} loan{activeLoans.length !== 1 ? "s" : ""} · avg{" "}
              <span className="font-semibold text-sky-600 dark:text-sky-400">
                {effectiveRate.toFixed(1)}%/mo
              </span>
            </p>
          </div>
          <div className="flex gap-1.5 mb-1">
            <Link
              to="/loans"
              className="w-8 h-8 flex items-center justify-center rounded-lg bg-sky-50 dark:bg-sky-900/20 text-sky-600 dark:text-sky-400 hover:bg-sky-100 dark:hover:bg-sky-900/30 transition-colors"
              title="Loans"
            >
            ...
            <Link
              to="/interest"
              className="w-8 h-8 flex items-center justify-center rounded-lg bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-900/30 transition-colors"
              title="Interest"
            >
        ...
        <div className="grid grid-cols-2 gap-3 mt-4">
          <div className="bg-emerald-50 dark:bg-emerald-900/15 border border-emerald-100 dark:border-emerald-800/30 rounded-xl px-3.5 py-3">
            ...
            <p className="text-lg sm:text-xl font-black text-emerald-700 dark:text-emerald-300 font-mono leading-none">
              {m(monthlyEarning, { decimals: 0 })}
            </p>
          </div>
          <div className="bg-sky-50 dark:bg-sky-900/15 border border-sky-100 dark:border-sky-800/30 rounded-xl px-3.5 py-3">
            <p className="text-[10px] text-sky-600 dark:text-sky-400 font-semibold mb-0.5">
              Expected Yearly Interest
            </p>
            <p className="text-lg sm:text-xl font-black text-sky-700 dark:text-sky-300 font-mono leading-none">
              {m(monthlyEarning * 12, { decimals: 0 })}
            </p>
          </div>
        </div>
      </motion.div>
      ...
      {portfolio.length > 0 && (
        <div>
          <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200 mb-2.5">
            Money Given Per Person
          </h3>
                ...
                <div key={entry.name} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">
                      {entry.loanCount} loan{entry.loanCount > 1 ? "s" : ""} · {pct.toFixed(0)}%
                      {entry.earned > 0 && <> · earned {m(entry.earned)}</>}
                    </p>
                  </div>
                </div>
              );
            ...
            <div className="space-y-3">
              {rateBreakdown.map(({ rate, principal, pct }) => (
                <div key={rate} className="flex items-center gap-3">
                  <span className="text-sm font-black text-sky-600 dark:text-sky-400 font-mono w-12 shrink-0">
                    {rate}%
                  </span>
                  <div className="flex-1">
                    ...
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">
                      earns {m((principal * rate) / 100, { decimals: 0 })}/mo
                    </p>
                  </div>
                </div>
              ))}
                ...
                return (
                  <div key={type} className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      <div
                        className={`w-2.5 h-2.5 rounded-full ${type === "Gold" ? "bg-amber-400" : type === "Property" ? "bg-emerald-400" : type === "Vehicle" ? "bg-orange-400" : "bg-slate-400"}`}
                      />
      ...
      {projection.length > 0 &&
        (() => {
          ...
          const points = projection.map((p, i) => {
            ...
            const y =
              padTop + usableH - ((p.value - totalPrincipal) / (maxVal - totalPrincipal)) * usableH;
            return { x, y, ...p };
          });
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200">
                    If You Reinvest Earnings
                  </h3>
                  <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">
                    5-year projection at current {effectiveRate.toFixed(1)}%/mo rate
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] text-slate-400 dark:text-slate-500">Year 5</p>
                  <p className="text-base sm:text-lg font-black text-emerald-600 dark:text-emerald-400 font-mono leading-none">
                    {short(projection[5]?.value ?? 0)}
                  </p>
                </div>
              </div>
                ...
                >
                  ...
                  {[1, 2, 3, 4].map((i) => (
                    <line
                      key={i}
                      x1="0"
                      y1={padTop + (usableH / 4) * i}
                      x2={chartW}
                      y2={padTop + (usableH / 4) * i}
                      stroke="currentColor"
                  ...
                  {points.map((p, i) => (
                    <circle
                      ...
                      cy={p.y}
                      r={i === 0 ? 0.8 : 1.2}
                      fill={i === 0 ? "rgb(148,163,184)" : "rgb(16,185,129)"}
                    />
                  ))}
                </svg>
              ...
              <div className="grid grid-cols-5 gap-1.5 mt-3 pt-3 border-t border-slate-100 dark:border-slate-700/40">
                {projection.slice(1).map((p) => (
                  <div key={p.year} className="text-center">
                    <p className="text-xs sm:text-sm font-black text-slate-900 dark:text-white font-mono leading-none">
                      {short(p.value)}
                    </p>
            ...
            {recentActivityGroups.map((group) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    setActivityPopover(activityPopover === group.loanId ? null : group.loanId)
                  }
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] text-slate-400 dark:text-slate-500">
                      ...
                      record{group.entries.length !== 1 ? "s" : ""}
                    </p>
                  </div>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-[11px] text-slate-400 dark:text-slate-500">
                              {borrower?.fullName || "Unknown"} · {loan?.rate}%/mo
                            </p>
                          </div>
                          ...
                          {group.entries.map((interest) => (
                            ...
                            >
                              <div className="text-[11px] font-mono text-slate-500 dark:text-slate-400">
                                {format(addDays(new Date(interest.startDate), 1), "dd MMM")} →{" "}
                                {format(new Date(interest.endDate), "dd MMM yy")}
                              </div>
                              <span className="text-xs font-black text-emerald-600 dark:text-emerald-400 font-mono ml-3 shrink-0">
                                +{m(interest.amount)}
                              </span>
                            </div>
                          ))}
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 font-mono">
                        {l.collateralCode || "—"} · {format(new Date(l.startDate), "dd MMM yy")} ·{" "}
                        {l.rate}%
                      </p>
                    </div>
                    <div className="flex items-center gap-2 ml-3 shrink-0">
                      ...
                      {l.status === "closed" && (
                        <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-700 text-slate-400">
                          CLOSED
                        </span>
                      )}
                    </div>
